...
float convolution(float ***global, int dimension_x, int dimension_y, int local_dimension_x, int local_dimension_y, 
	int proc_per_x, int proc_per_y, float filter[FILTER_SIZE][FILTER_SIZE], int rank, int num_of_proc, MPI_Comm comm, int num_of_threads , int *reps ){
...
*/
		...
		Calcutalate_InnerCells(&local, &new_local, local_dimension_x, local_dimension_y, filter, num_of_threads);  
		...
		while(k < 8){
			...
			if(flag != 0){			
				...
				if(status.MPI_SOURCE == top_rank && !top){
					...
					Calcutalate_TopRow(&local, &new_local, local_dimension_x, local_dimension_y, filter, rows[0], no_top, num_of_threads);
					top = true;
				}
					
			 	else if(status.MPI_SOURCE == bot_rank && !bot){
					memmove(&rows[1], &buffer, sizeof(float)*local_dimension_y );
					Calcutalate_BotRow(&local, &new_local, local_dimension_x, local_dimension_y, filter, rows[1], no_bot, num_of_threads);
					bot = true;
			 	}
						
			 	else if(status.MPI_SOURCE == left_rank && !left){
					memmove(&columns[0], &buffer, sizeof(float)*local_dimension_x );
					Calcutalate_LeftColumn(&local, &new_local, local_dimension_x, local_dimension_y, filter, columns[0], no_left, num_of_threads);
					left = true;
			 	}
						
			 	else if(status.MPI_SOURCE == right_rank && !right){
					memmove(&columns[1], &buffer, sizeof(float)*local_dimension_x );
					Calcutalate_RightColumn(&local, &new_local, local_dimension_x, local_dimension_y, filter, columns[1], no_right, num_of_threads);
					right = true;
			 	}
					...
					if(top && left && !top_left){
						Calcutalate_TopLeftCell(&local, &new_local, local_dimension_x, local_dimension_y, filter, rows[0], columns[0], corners_buff[0],
							no_top, no_left, no_top_left, num_of_threads);
						top_left = true;
					}
			 	}
					...
					if(top && right && !top_right){
						Calcutalate_TopRightCell(&local, &new_local, local_dimension_x, local_dimension_y, filter, rows[0], columns[1], corners_buff[1],
							no_top, no_right, no_top_right, num_of_threads);
						top_right = true;
					}
			 	}
					...
					if(bot && left && !bot_left){
						Calcutalate_BotLeftCell(&local, &new_local, local_dimension_x, local_dimension_y, filter, rows[1], columns[0], corners_buff[2],
							no_bot, no_left, no_bot_left, num_of_threads);
						bot_left = true;
					}
			 	}
					...
					if(bot && right && !bot_right){
						Calcutalate_BotRightCell(&local, &new_local, local_dimension_x, local_dimension_y, filter, rows[1], columns[1], corners_buff[3],
							no_bot, no_right, no_bot_right, num_of_threads);
						bot_right = true;
					}
				}
		 	}
...
omp_set_num_threads(num_of_threads);
#pragma omp parallel for shared (grid, new_grid, dimension_x, dimension_y, filter) private(i, j) collapse(2)
...
static inline void Calcutalate_ExternalCells(float ***grid, float ***new_grid, int dimension_x, int dimension_y, 
	...
	bool no_top, bool no_bot, bool no_left, bool no_right, bool no_top_left, bool no_top_right, bool no_bot_left, bool no_bot_right, int num_of_threads){
	...
	omp_set_num_threads(num_of_threads);
#pragma omp parallel for shared (new_grid, grid, dimension_x,dimension_y, filter, top_buff) private(j)
	for(j=1; j< dimension_y-1; j++){
	...
	omp_set_num_threads(num_of_threads);
#pragma omp parallel for shared (new_grid, grid, dimension_x,dimension_y, filter, no_left, no_right) private(j)
...
#pragma omp parallel shared (new_grid, grid,filter,dimension_y,dimension_x,top,top_left, top_right, left, right, bot, bot_left, bot_right )
	{
	#pragma omp sections 
		{
			#pragma omp section
...
static inline void Calcutalate_TopRow(float ***grid, float ***new_grid,int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float top_buff[], bool no_top, int num_of_threads){
	int j;
	omp_set_num_threads(num_of_threads);
...
static inline void Calcutalate_BotRow(float ***grid, float ***new_grid, int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float bot_buff[], bool no_bot, int num_of_threads){
...
static inline void Calcutalate_LeftColumn(float ***grid, float ***new_grid, int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float left_buff[], bool no_left, int num_of_threads){ 	
	int j;
	omp_set_num_threads(num_of_threads);
#pragma omp parallel for //private(j)
	for(j=1; j< dimension_x-1; j++){
...
static inline void Calcutalate_RightColumn(float ***grid, float ***new_grid, int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float right_buff[], bool no_right, int num_of_threads){
...
static inline void Calcutalate_TopLeftCell(float ***grid, float ***new_grid, int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float top_buff[], float left_buff[], float top_left_value, bool no_top, bool no_left, bool no_top_left, int num_of_threads){
...
static inline void Calcutalate_TopRightCell(float ***grid, float ***new_grid, int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float top_buff[], float right_buff[], float top_right_value, bool no_top, bool no_right, bool no_top_right, int num_of_threads){
...
static inline void Calcutalate_BotLeftCell(float ***grid, float ***new_grid, int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float bot_buff[], float left_buff[], float bot_left_value, bool no_bot, bool no_left, bool no_bot_left, int num_of_threads){
...
static inline void Calcutalate_BotRightCell(float ***grid, float ***new_grid,int dimension_x, int dimension_y,
	float filter[FILTER_SIZE][FILTER_SIZE], float bot_buff[], float right_buff[], float bot_right_value, bool no_bot, bool no_right, bool no_bot_right, int num_of_threads){	
