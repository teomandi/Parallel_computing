...
int main(int argc, char** argv) {
	...
	int num_of_proc, rank, i, j, num_of_threads=-1;
	...
	if(num_of_threads == -1){ //if its not given
	...
	*/
	...
	for(i = 0; i < bytes_per_pixel; i++) //convolution for each grid containing the rgb bytes -- if the image is colorfull
		time[i] = convolution(&(global[i]), dimension_x, dimension_y, local_dimension_x, local_dimension_y,  
			proc_per_x, proc_per_y, filter, rank, num_of_proc, comm, num_of_threads,  &reps[i]);
	...
	if(rank==0){
		...
		if(new_filename != NULL)
			...
			new_file =  fopen("../produced_images/Hybrid_produced.raw", "w+");
		...
		printf("Duration of Convolution: %.5f sec\nProcesses: %d - with Threads: %d \n", time[0]+time[1]+time[2], num_of_proc, num_of_threads);
